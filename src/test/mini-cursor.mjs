...
- 禁止使用 echo、管道符 | 或任何交互式输入 hack
- 所有命令必须是可直接执行的非交互命令
- 错误示例: { command: "cd react-todo-app && pnpm install", workingDirectory: "react-todo-app" }
- 正确示例: { command: "pnpm install", workingDirectory: "react-todo-app" }

重要规则 - write_file：
- 当写入 React 组件文件（如 App.tsx）时，如果存在对应的 CSS 文件（如 App.css），在这个组件的其他 import 语句后加上这个 css 的导入
`));
    ...
    for (let i = 0; i < maxIterations; i++) {
        ...
        for await (const chunk of rawStream) {
            ...
            try {
                ...
                console.log('需要调用的工具', parsedTools);
            } catch (e) {
        ...
        console.log('存入的AIMessage', fullAIMessage);
